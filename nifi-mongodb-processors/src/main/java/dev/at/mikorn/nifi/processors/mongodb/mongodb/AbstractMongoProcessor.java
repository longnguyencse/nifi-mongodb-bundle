...
public abstract class AbstractMongoProcessor extends AbstractProcessor {
    ...
    public final void createClient(ProcessContext context) throws IOException {
        ...
        PropertyValue host = context.getProperty(HOSTS);
        PropertyValue databaseName = context.getProperty(DATABASE_NAME);
        List<ServerAddress> addresses = parseServerAddresses(context.getProperty(HOSTS)
...
//                ServerAddress addr = new ServerAddress("192.168.1.161", 27017);
                MongoCredential credential = MongoCredential
                        .createCredential(username.getValue(), databaseName.getValue(), password.getValue().toCharArray());
